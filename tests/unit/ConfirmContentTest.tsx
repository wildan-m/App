...
jest.mock('@components/ButtonComposed', () => {
    ...
    MockButton.Text = (props: {children?: React.ReactNode}) => ReactLib.createElement('mock-button-text', props);
    MockButton.KeyboardShortcut = () => null;
...
describe('ConfirmContent', () => {
    ...
    function getConfirmButtonProps(onConfirm: () => void): ButtonProps | undefined {
        return mockButtonSpy.mock.calls.find((call) => call[0].onPress === onConfirm)?.[0];
    }

    const testCases = [
        {shouldShowCancelButton: false, variant: undefined, expectedVariant: undefined},
        ...
        {shouldShowCancelButton: true, variant: undefined, expectedVariant: CONST.BUTTON_VARIANT.SUCCESS},
    ...
    it('forces the success variant when shouldUseSuccessStyleForConfirm is set without a cancel button', () => {
        const onConfirm = jest.fn();
        render(
            <ConfirmContent
                title="Test"
        ...
        expect(getConfirmButtonProps(onConfirm)?.variant).toBe(CONST.BUTTON_VARIANT.SUCCESS);
    });
});
