...
describe('ModifiedExpenseMessage', () => {
    ...
    describe('getForAction', () => {
        ...
        describe('when the date change recalculates the distance rate', () => {
            const reportAction = {
                ...createRandomReportAction(1),
                actionName: CONST.REPORT.ACTIONS.TYPE.MODIFIED_EXPENSE,
                originalMessage: {
                    oldMerchant: '56.36 mi @ $0.70 / mi',
                    merchant: '56.36 mi @ $0.50 / mi',
                    oldAmount: 3945,
                    amount: 2818,
                    oldCurrency: CONST.CURRENCY.USD,
                    currency: CONST.CURRENCY.USD,
                    created: '2026-01-01',
                    oldCreated: '2025-12-31',
                },
            };

            it('then the message reports both the rate change and the date change', () => {
                const expectedResult = `changed the rate to ${reportAction.originalMessage.merchant} (previously ${reportAction.originalMessage.oldMerchant}), which updated the amount to $28.18 (previously $39.45)\nchanged the date to 2026-01-01 (previously 2025-12-31)`;
                const result = getForReportAction({
                    translate: translateLocal,
                    reportAction,
                    policy: undefined,
