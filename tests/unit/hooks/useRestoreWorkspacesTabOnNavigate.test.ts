...
describe('useRestoreWorkspacesTabOnNavigate', () => {
    ...
    it('hydrates from sessionStorage when the live navigation tree has no workspace route', () => {
        ...
        // Mirror the real getStateFromPath output: WORKSPACE_NAVIGATOR is nested inside TAB_NAVIGATOR
        // (per the linking config), not at the top level. The earlier shape masked the
        // sessionStorage-fallback bug because it skipped this wrapper.
