...
describe('LeaveWorkspaceFlow', () => {
    ...
    it('shows a single-button success modal when the user is the reimbursement contact', async () => {
        ...
        renderAction();
        await waitForBatchedUpdatesWithAct();
        expect(mockShowConfirmModal).toHaveBeenCalledWith(expect.objectContaining({variant: CONST.BUTTON_VARIANT.SUCCESS, shouldShowCancelButton: false}));
    });
    ...
    it('shows a danger two-button modal when the user is not the reimbursement contact', async () => {
        ...
        renderAction();
        await waitForBatchedUpdatesWithAct();
        expect(mockShowConfirmModal).toHaveBeenCalledWith(expect.objectContaining({variant: CONST.BUTTON_VARIANT.DANGER}));
    });
