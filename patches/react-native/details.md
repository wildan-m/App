...
### [react-native+0.86.0+042+clamp-blob-range-APP-KDN.patch](react-native+0.86.0+042+clamp-blob-range-APP-KDN.patch)

- Reason: Fixes a fatal iOS crash (APP-KDN, `NSRangeException: -[NSConcreteMutableData subdataWithRange:]: range {0, 321} exceeds data length 318`). `-[RCTBlobManager resolve:offset:size:]` builds `NSMakeRange(offset, size)` straight from the JavaScript blob descriptor and never compares it with `data.length`. The descriptor and the stored bytes are produced independently — JavaScript tallies the size itself in `BlobManager.createFromParts`, while native assembles the bytes in `-createFromParts:withId:`, where a part that fails to resolve appends nothing because `-[NSMutableData appendData:]` treats `nil` as a no-op — so a blob can legitimately be recorded as longer than it is stored. When that short blob is later used as a part of another blob, the unchecked range raises an `NSRangeException` on the module's method queue, where there is no JavaScript frame to catch it and the app terminates. The same method also mishandles the "read to the end" sentinel: `size == -1` combined with a non-zero offset produces `NSMakeRange(offset, NSUIntegerMax)`, which is a guaranteed crash and is reachable from `-resolveURL:` via a `blob:` URL that carries `offset` but no `size`. Android has neither problem: `BlobModule.resolve` normalises `-1` to `data.size - offset` and uses `Arrays.copyOfRange`, which pads instead of throwing. The patch clamps the offset and size to the bytes actually stored, normalises a negative size the way Android does, and calls `RCTLogError` with the blob ID and both lengths so the desync is diagnosable instead of silent. It also makes `-handleNetworkingRequest:` nil-safe: `-resolve:` already returns `nil` for a released blob, and an `NSDictionary` literal raises `NSInvalidArgumentException` on a `nil` value, so the upload path could trade one fatal exception for another — the same nil-in-a-collection-literal hazard already patched for `@[...]` in `react-native+0.86.0+030+fix-nil-BlobModule-crash-APP-8BM.patch`.
- Upstream PR/issue: 🛑
