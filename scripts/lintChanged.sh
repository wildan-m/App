...
if [[ -n "$GIT_DIFF_OUTPUT" ]] ; then
    ...
    eslint --concurrency=auto --max-warnings=145 --cache --cache-location=node_modules/.cache/eslint-changed --cache-strategy content --config ./config/eslint/eslint.changed.config.mjs $GIT_DIFF_OUTPUT
else
