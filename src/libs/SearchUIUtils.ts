...
function getSuggestedSearchesVisibility(
    ...
    hasReportsAwaitingApproval = false,
    ...
    // Keep the "Needs approval" suggestion visible whenever the user still has reports awaiting their approval,
    // even if the current workspace approval config no longer lists them as an approver (e.g. after an approver change).
    let shouldShowApproveSuggestion = hasReportsAwaitingApproval;
...
type TypeMenuSectionsParams = {
    ...
    hasReportsAwaitingApproval?: boolean;
};
...
function createTypeMenuSections(params: TypeMenuSectionsParams): SearchTypeMenuSection[] {
    ...
    } = getSuggestedSearchesVisibility(currentUserEmail, cardFeedsByPolicy, policies, defaultExpensifyCard, hasReportsAwaitingApproval);
