import {buildTravelDotURL, isTravelTestAccountResolved, openTravelDotLink as openTravelDotLinkWeb} from '@libs/actions/Link';
...
const openTravelDotLink = (activePolicyID?: string, postLoginPath?: string, spotnanaToken?: string, isTestAccount?: boolean) => {
    if (spotnanaToken) {
        asyncOpenURL(Promise.resolve(), () => buildTravelDotURL(spotnanaToken, isTravelTestAccountResolved(isTestAccount), postLoginPath));
        return;
    }
