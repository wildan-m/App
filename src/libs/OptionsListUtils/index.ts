...
function getLastMessageTextForReport({
    ...
    if (isArchivedNonExpenseReport(report, isReportArchived)) {
        ...
        lastMessageTextFromReport = translate('violations.smartscanFailed', {canEdit: wasActionTakenByCurrentUser(iouAction), missingFields: getOriginalMessage(lastReportAction)?.missingFields ?? []});
