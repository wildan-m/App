...
import saveQueuedReceiptsToGallery from '@libs/saveQueuedReceiptsToGallery';
...
import {getPendingQueuedReceipts, logReceiptQueueSnapshot} from '@libs/telemetry/ReceiptObservability';
...
async function signOut(params: {autoGeneratedLogin?: string; signedInWithSAML?: boolean; authToken?: string} = {}): Promise<void | Response<never>> {
    ...
    // Read the still-queued receipts now, while the queue is intact. cleanupSession() clears the persisted requests
    // and redirectToSignIn() clears Onyx, so after this point nothing knows where those images live. The copy itself
    // is allowed to finish after we sign out, because the durable receipts folder is not touched by sign-out cleanup.
    saveQueuedReceiptsToGallery(getPendingQueuedReceipts());
