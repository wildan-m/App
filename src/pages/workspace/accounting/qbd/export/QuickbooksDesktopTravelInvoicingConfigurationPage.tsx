...
function QuickbooksDesktopTravelInvoicingConfigurationPage({policy}: WithPolicyConnectionsProps) {
    ...
    const travelVendor = vendors?.find((v) => v.id === qbdConfig?.travelInvoicingVendorID);
    const travelPayableAccount = payableAccounts?.find((a) => a.id === qbdConfig?.travelInvoicingPayableAccountID);

    const sections: QBDSectionType[] = [
        {
            title: travelVendor?.name,
            description: translate('workspace.qbd.travelInvoicingVendor'),
            onPress: () => {
                if (!policyID) {
                    return;
                }
                Navigation.navigate(ROUTES.POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_TRAVEL_INVOICING_VENDOR_SELECT.getRoute(policyID));
            },
        ...
        },
        {
            ...
            description: translate('workspace.qbd.travelInvoicingPayableAccount'),
            onPress: () => {
                if (!policyID) {
                    return;
                }
                Navigation.navigate(ROUTES.POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_TRAVEL_INVOICING_PAYABLE_ACCOUNT_SELECT.getRoute(policyID));
            },
    ...
    return (
        <ConnectionLayout
            ...
            headerTitle="workspace.qbd.travelInvoicing"
