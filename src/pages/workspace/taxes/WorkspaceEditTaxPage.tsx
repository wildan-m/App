...
function WorkspaceEditTaxPage({
    ...
    // `canEditTaxRate` gates the actions that must not happen on the workspace default rate: disabling it and deleting it.
    const canEditTaxRate = canWriteTaxes && policy && canEditTaxRateUtil(policy, currentTaxID ?? taxID);
    // The Name, Value and Tax Code fields are always editable when the user has write access — being the default
    // rate should not lock these fields.
