    ...
    for (const policy of workspaceListPolicies ?? []) {
        if (policy.isJoinRequestPending && policy.nonMemberDetails) {
            ...
            const pendingWorkspaceRow: WorkspaceRowData = {
                ...
                ownerAccountID,
                ownerLogin: ownerDetails?.login ?? policy.nonMemberDetails.ownerEmail,
                ownerAvatar: ownerDetails?.avatar ?? policy.nonMemberDetails.avatar,
                ownerName: ownerDetails ? getDisplayNameOrDefault(ownerDetails) : policy.nonMemberDetails.ownerEmail,
