...
function ConfirmationStep({policyID, stepNames, startStepIndex}: ConfirmationStepProps) {
    ...
    // Mirror the spend rule step-inclusion condition from LimitTypeStep so the Restriction field is only shown
    // when the Spend Rules step is actually part of the flow, not just because the stale spendRuleEnabled flag is set.
    const isSpendRuleAvailable = !isProduction && isPolicyFeatureEnabled(policy, CONST.POLICY.MORE_FEATURES.ARE_RULES_ENABLED);
    const isSpendRuleApplied =
        !!issueNewCard?.data.spendRuleEnabled && (isSpendRuleAvailable || issueNewCard?.data.cardType === CONST.EXPENSIFY_CARD.CARD_TYPE.VIRTUAL);
