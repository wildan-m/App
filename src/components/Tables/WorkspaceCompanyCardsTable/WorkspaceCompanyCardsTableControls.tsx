...
function WorkspaceCompanyCardsTableControls({policyID, domainOrWorkspaceAccountID, bankName, canWriteCompanyCards, clearCardSelection}: WorkspaceCompanyCardsTableControlsProps) {
    ...
    const isSingleAssignedCardSelected = selectedAssignedCards.length === 1;
    ...
    const confirmBulkUnassign = async () => {
        ...
        const {action} = await showConfirmModal({
            ...
            title: translate(isSingleAssignedCardSelected ? 'workspace.moreFeatures.companyCards.unassignCard' : 'workspace.moreFeatures.companyCards.unassignCards'),
            prompt: translate(isSingleAssignedCardSelected ? 'workspace.moreFeatures.companyCards.unassignCardDescription' : 'workspace.moreFeatures.companyCards.unassignCardsDescription'),
        ...
        if (isOnlyAssignedCardsSelected) {
            if (canWriteCompanyCards) {
                options.push({
                    ...
                    text: translate(isSingleAssignedCardSelected ? 'workspace.moreFeatures.companyCards.unassignCard' : 'workspace.moreFeatures.companyCards.unassignCards'),
                    value: 'unassign',
