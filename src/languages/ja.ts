...
${integrationName === CONST.ONBOARDING_ACCOUNTING_MAPPING.other ? 'あなたの' : '宛先'} ${integrationName} を接続して、自動経費コード設定と同期を行い、月末締めをスムーズにしましょう。
    ...
    statusPage: {
        ...
        chooseDelegate: '代理人を選択',
        vacationDelegateClearAfter: 'クリア日（推奨）',
        vacationDelegateWillClearOn: (date: string) => `休暇代理人は次の日に解除されます: ${date}`,
        vacationDelegateUntil: (date: string) => `期限: ${date}`,
